...
def open_host(request: Request) -> dict:
    runtime = get_browser_runtime(request)
    try:
        state = runtime.relaunch_visible_host()
        store_preview_cache(request, None, None)
        _record_action(request, tool="visualizer.open_host", message="visible host opened for human takeover")
        payload = build_visualizer_state(request).model_dump()
        payload["browser_state"] = state.model_dump()
        payload["runtime"]["visible_window"] = True
        return payload
    except BrowserHostClosedError as exc:
        _record_action(request, tool="visualizer.open_host", status="error", code="browser_host_closed", message=str(exc))
        raise HTTPException(status_code=503, detail={"code": "browser_host_closed", "message": str(exc)}) from exc
    except Exception as exc:
        _record_action(request, tool="visualizer.open_host", status="error", message=str(exc))
        raise HTTPException(status_code=500, detail=str(exc)) from exc
